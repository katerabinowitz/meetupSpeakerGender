...
# filter to necessary columns (nonBinary was removed because it's zero
# throughout and will be noted in graphs) and only applicable meetups
...
meetup['g'] = meetup['group'].str.replace('Washington DC Area ', '').str.replace(
    ' DC', '').str.replace('DC ', '').str.replace('D.C. ', '')
meetup['g'] = meetup['g'].str.replace(' etc', '').str.replace(
    ' Professionals', '').str.replace(' Community', '')
...
# identify multi vs. single speaker events and sum gender for each meetup
# type and group
meetup['singleSpeaker'] = np.where(
    meetup['male'] + meetup['female'] == 1, 'yes', 'no')

meetupSum = meetup.groupby(['g', 'singleSpeaker']).sum().add_prefix('sum_')
meetupSum = meetupSum.rename(
    columns={'sum_male': 'Male', 'sum_female': 'Female'})

meetupSingle = meetupSum.loc[
    (meetupSum.index.get_level_values('singleSpeaker') == 'yes')]
meetupSingle.index = meetupSingle.index.droplevel(1)
# Appears DC-specific
# meetupSingle.index = np.where(
#     meetupSingle.index == "NodeDC", "node.dc", meetupSingle.index)
meetupSingle.index.name = 'g'

meetupMulti = meetupSum.loc[
    (meetupSum.index.get_level_values('singleSpeaker') == 'no')]
meetupMulti.index = meetupMulti.index.droplevel(1)

# output
meetupSingle.to_csv('SingleSpeaker.csv')
meetupMulti.to_csv('MultipleSpeakers.csv')
